...
import com.wotin.geniustest.databinding.ActivityLoginBinding
...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mBinding.goToSignupActivityButton.setOnClickListener {
    ...
    fun clickedLoginButton() {
        ...
        if(networkState(connectivityManager)) {
            ...
            if (mBinding.loginIdEdittext.text!!.isNotEmpty() && mBinding.loginPasswordEdittext.text!!.isNotEmpty() && mBinding.loginIdEdittext.text!!.trim().length >= 7 && mBinding.loginPasswordEdittext!!.text!!.length >= 8) {
                signInAndSignUpApiService.signIn(
                    id = EncryptionAndDetoxification()
                        .encryptionAndDetoxification(mBinding.loginIdEdittext.text.toString()),
                    password = EncryptionAndDetoxification()
                        .encryptionAndDetoxification(mBinding.loginPasswordEdittext.text.toString())
            ...
            } else if(mBinding.loginIdEdittext.text!!.isEmpty() || mBinding.loginPasswordEdittext.text!!.isEmpty())
                Toast.makeText(applicationContext, "id 와 password 를 입력해주세요", Toast.LENGTH_LONG).show()
            else if(mBinding.loginIdEdittext.text!!.trim().length < 7 || mBinding.loginPasswordEdittext.text!!.length < 8)
